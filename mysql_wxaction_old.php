...
SUM(jcsl-ccsl+tzjcsl-tzccsl) AS kcsl,SUM(jczl-cczl+tzjczl-tzcczl) AS kczl
...
function menusystemlist() {
   ...
   if ($khsystem=="1")
   {
	$sqlstr = " SELECT menu,sort,name,iconurl,viewpath ,menu_id,widgetName FROM menusystem where alluser=1 or ( enabled=1 and khbz=2  and instr(termtype,'" . $termtype . "')>0  ) ";
...
function cpxsdmxsave() {
   ...
   $khid=$o->{'khid'};
	...
	$result = mysql_query($dhsql);
	//return mysql_errno().$dhsql;
	$arr=mysql_fetch_assoc($result);
	//return $arr.'-'.$dhsql;
	$khmc =$arr['khmc'];
  
//return $khmc.'--'.$khid.'=='. $o->{'ckmc'};
...
case 1 :
//insert
$sql = "insert into Customer(C_code,L_id,C_name,Address,Tel,Py_code,password,C_shortname) values('" . $arr['C_code'] . "'";
$sql .= "," . $arr['L_id'];
$sql .= ",'" . $arr['C_name'] . "'";
$sql .= ",'" . $arr['Address'] . "'";
$sql .= ",'" . $arr['Tel'] . "'";
...
function userssave($optype) {
	...
	foreach ($params as $arr) {
	 ...
	 case 1 :
		if ($table=="khusers"){
			$sql = "insert into khusers(khid,edit,sh,cwsh,del,new,system,usercode,username,tel,sfjhm,qqnumber,
...
//return $sqlstr;
	...
	if (($cpkc)&&($ckquery)) {
		...
		$menu_ck = array();
		$menu_cpkc = array();
		while ($menurow = mysql_fetch_array($ckquery)) {
			...
			$menu_array1 = array();
			mysql_data_seek($cpkc, 0);
			while ($row = mysql_fetch_array($cpkc)) {
				if ($row['ckid'] == $menutype) {
					$my_array = array();
					for ($i = 0; $i < mysql_num_fields($cpkc); $i++) {
						$newvar = $row[mysql_field_name($cpkc, $i)];
						$my_array[mysql_field_name($cpkc, $i)] = urlencode($newvar);
					};
					array_push($menu_array1, $my_array);
				}
			}
...
function wxmenulist() {
	...
	$psort=$_GET["psort"];
	$sqlstr = " SELECT Psort,Menu,grou,Sort,Name,Page FROM wxmenu where Active=1 and psort='".$psort;
	$sqlstr .= "' order by Psort,grou,Sort ";
   //return $sqlstr; 
	$query = mysql_query($sqlstr);
	
	$sqlstr1 = " SELECT Psort,Menu,Description FROM wxmenu where Active=1 and psort='".$psort."' group by Psort  order by Psort  ";
	$typequery = mysql_query($sqlstr1);
	if ($query) {
		$menu_array = array();
		$menu = array();
...
function cpxsdlist() {	
	...
	//	return $sqlstr;
		...
		while ($xsdrow = mysql_fetch_array($xsdquery)) {
			$id = $xsdrow['xsid'];
			$my_array = array();
			for ($i = 0; $i < mysql_num_fields($xsdquery); $i++) {
						$newvar = $xsdrow[mysql_field_name($xsdquery, $i)];
						$my_array[mysql_field_name($xsdquery, $i)] = urlencode($newvar);
			};
